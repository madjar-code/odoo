...
class MailBot(models.AbstractModel):
    ...
    def _get_answer(self, record, body, values, command=False):
        # onboarding
        erpbot_state = self.env.user.erpbot_state
        if self._is_bot_in_private_channel(record):
            # main flow
            if erpbot_state == 'onboarding_emoji' and self._body_contains_emoji(body):
                self.env.user.erpbot_state = "onboarding_command"
            ...
            elif erpbot_state == 'onboarding_command' and command == 'help':
                self.env.user.erpbot_state = "onboarding_ping"
                self.env.user.odoobot_failed = False
            ...
            elif erpbot_state == 'onboarding_ping' and self._is_bot_pinged(values):
                self.env.user.erpbot_state = "onboarding_attachement"
                self.env.user.odoobot_failed = False
                return _("Yep, I am here! 🎉 <br/>Now, try <b>sending an attachment</b>, like a picture of your cute dog...")
            elif erpbot_state == 'onboarding_attachement' and values.get("attachment_ids"):
                self.env.user.erpbot_state = "idle"
                self.env.user.odoobot_failed = False
            ...
            elif erpbot_state in (False, "idle", "not_initialized") and (_('start the tour') in body.lower()):
                self.env.user.erpbot_state = "onboarding_emoji"
                return _("To start, try to send me an emoji :)")
            # easter eggs
            elif erpbot_state == "idle" and body in ['❤️', _('i love you'), _('love')]:
            ...
            # help message
            elif self._is_help_requested(body) or erpbot_state == 'idle':
                ...
                # repeat question
                if erpbot_state == 'onboarding_emoji':
                ...
                elif erpbot_state == 'onboarding_ping':
                    self.env.user.odoobot_failed = True
