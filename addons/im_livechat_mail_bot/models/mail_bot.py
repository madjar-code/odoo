...
class MailBot(models.AbstractModel):
    ...
    def _get_answer(self, record, body, values, command):
        ...
        if self._is_bot_in_private_channel(record):
            if erpbot_state == "onboarding_attachement" and values.get("attachment_ids"):
                self.env.user.odoobot_failed = False
                self.env.user.erpbot_state = "onboarding_canned"
                return _("That's me! 🎉<br/>Try typing <span class=\"o_odoobot_command\">:</span> to use canned responses.")
            elif erpbot_state == "onboarding_canned" and values.get("canned_response_ids"):
                self.env.user.odoobot_failed = False
            ...
            elif erpbot_state == 'onboarding_canned' and not self._is_help_requested(body):
