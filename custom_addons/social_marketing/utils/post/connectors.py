...
class FacebookConnector(ConnectorInterface):
    ...
    def flex_create_post_test(self) -> PostType:
        graph = facebook.GraphAPI(access_token=self._access_token)
        image_ids = ['122099841260055468', '122099842838055468']
        post = {
            'message': '123456',
            'attached_media': [{'media_fbid': image_id} for image_id in image_ids]
        }
        graph.put_object(parent_object='me', connection_name='feed', **post)
    ...
    def flex_create_post(self) -> PostType:
        ...
        #     response = graph.put_photo(image=image_object.image)
        #     media_id = response['id']
        #     media_ids.append({'photoId': str(media_id)})
